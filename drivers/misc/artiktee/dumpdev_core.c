 ...
 * with this program.
 *
 * Author: <aron.kwon@samsung.com>
 * <jklolo.lee@samsung.com>, <jong-tak.lee@samsung.com>
 *********************************************************/
...
/* TODO : will be remove this declare.
 *   and should be share tzsys.c's dump path */
...
static int copy_to_file_from_ring(struct chimera_ring_buffer *src_ring,
				  int src_ring_size,
				  char *file_full_path, int data_order)
{
	struct iovec iov[2];
	int i;
	size_t n_written = 0;
	...
	for (i = 0; i < nvec; ++i) {
		...
		if (data_order == 1 && i == 0) {
			if (ss_file_create_object(file_full_path,
			    ...
			    iov[i].iov_len) {
				tzlog_print(K_ERR,
					    "Fail - ss_file_create_object(%s)(req:%d)\n",
					    file_full_path, iov[i].iov_len);
				break;
			}
		} else {
			if (ss_file_append_object(file_full_path,
			    ...
			    iov[i].iov_len) {
				tzlog_print(K_ERR,
					    "Fail - ss_file_append_object(req:%d)\n",
					    iov[i].iov_len);
				break;
			}
		}
...
void dumpdev_handler(NSRPCTransaction_t *txn_object)
{
	...
	if (ring == NULL || ring_size == 0) {
		tzlog_print(K_ERR,
			    "NSRPC: can not process : ring : %p ring_size : %d\n",
			    ring, ring_size);
		ret = -1;
	...
	case NSRPC_CMD_DUMP_END:
		{
			...
			char *file_full_path =
			    get_file_full_path(dumpdev_ctrl->dump_file_name,
					       full_full_path_buf,
					       sizeof(full_full_path_buf));
			if (command == NSRPC_CMD_DUMP_TRANSFER)
				tzlog_print(K_DEBUG,
					    "NSRPC_CMD_DUMP_TRANSFER Received\n");
			else
				tzlog_print(K_DEBUG,
					    "NSRPC_CMD_DUMP_END Received\n");

			src_size = chimera_ring_buffer_readable(ring);
			if (src_size > 0) {
				int read_size = 0;

				read_size = copy_to_file_from_ring(ring,
								   ring_size,
								   file_full_path,
								   dumpdev_ctrl->data_order);
				if (src_size != read_size) {
					tzlog_print(K_ERR,
						    "Fail - copy_to_file_from_ring\n");
					ret = -1;
					break;
				}

				tzlog_print(K_INFO,
					    "NSRPC: file name %s order %d (src_size : %d)\n",
					    file_full_path,
					    dumpdev_ctrl->data_order, src_size);
			}
	...
	default:
		tzlog_print(K_WARNING,
			    "NSRPC: Received unsupported command %x\n",
			    command);
		ret = -1;
		break;
	}
...
exit_dumpdev_handler:
	...
	tzlog_print(K_DEBUG,
		    "NSRPC: Finished handling dumpdev command %d(ret:%d)\n",
		    command, ret);
}
